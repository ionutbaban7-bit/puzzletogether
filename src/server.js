...
function roomView(room) {
  return {
    ...
    pauseRequested: !!room.pauseRequested,
...
function applyHarvest(room, playerId, msg, ws) {
  ...
  if (!player || (player.role === "spectator" && !isHost)) return send(ws, { t: "error", code: "spectator", message: "Only participants and the facilitator can edit team notes." });
  if (!room.startedAt || !["debrief", "harvest"].includes(room.stage)) return send(ws, { t: "error", code: "harvest_stage", message: "Team notes can be edited during debrief and action planning." });
  const conflict = () => send(ws, { t: "error", code: "harvest_conflict", message: "This field changed while you were editing. Your draft is kept; compare it with the team's version before saving again." });
  // Atomic changes preserve colleagues' work in other fields. The expected
  // value also prevents a late blur from silently replacing the same field.
  if (msg.kind === "insight") {
    if (!["observed", "learned", "tryNext"].includes(msg.key) || typeof msg.value !== "string") return;
    if (msg.previous !== (room.insights[msg.key] || "")) return conflict();
    room.insights[msg.key] = msg.value.slice(0, 4000);
  } else if (msg.kind === "debriefField") {
    if (!Number.isInteger(msg.index) || msg.index < 0 || msg.index >= 10 || typeof msg.value !== "string") return;
    if (msg.previous !== (room.debriefNotes[msg.index] || "")) return conflict();
    room.debriefNotes[msg.index] = msg.value.slice(0, 2000);
  } else if (msg.kind === "actionAdd") {
    if (room.actions.length >= 20) return send(ws, { t: "error", code: "action_limit", message: "This session already has 20 actions." });
    room.actions.push({ id: crypto.randomUUID(), text: "", ownerId: "", due: "", done: false });
  } else if (msg.kind === "actionField") {
    const action = room.actions.find((item) => item.id === msg.id);
    if (!action || !["text", "ownerId", "due", "done"].includes(msg.key)) return;
    if (msg.previous !== action[msg.key]) return conflict();
    if (msg.key === "text" && typeof msg.value === "string") action.text = msg.value.slice(0, 500);
    else if (msg.key === "ownerId" && (msg.value === "" || room.knownPlayers.has(msg.value))) action.ownerId = msg.value;
    else if (msg.key === "due" && (msg.value === "" || /^\d{4}-\d{2}-\d{2}$/.test(msg.value))) action.due = msg.value;
    else if (msg.key === "done" && typeof msg.value === "boolean") action.done = msg.value;
    else return;
  } else if (msg.kind === "actionRemove") {
    const action = room.actions.find((item) => item.id === msg.id);
    if (!action) return;
    if (!msg.previous || ["text", "ownerId", "due", "done"].some((key) => msg.previous[key] !== action[key])) return conflict();
    room.actions = room.actions.filter((item) => item.id !== msg.id);
  } else {
    // Full-document writes from old clients can erase concurrent edits.
    return send(ws, { t: "error", code: "client_update_required", message: "Refresh this page to use collaborative team notes." });
  }
  touch(room);
  broadcastRoom(room);
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    switch (msg.t) {
      ...
      case "rating": {
        const { room, playerId } = attached;
        if (room.coachingActivity?.mode !== "questionnaire" || room.stage !== "play" || room.boardLocked || room.players.get(playerId)?.role === "spectator") break;
        const answers = {};
      ...
      case "pauseRequest": {
        const { room, playerId } = attached;
        if (!["brief", "play", "reveal", "debrief", "harvest"].includes(room.stage) || !room.players.has(playerId)) break;
        if (!room.conns.has(room.hostId)) { send(ws, { t: "error", code: "facilitator_offline", message: "The facilitator is offline. You can step away and rejoin when ready." }); break; }
        if (!room.pauseRequested) { room.pauseRequested = true; broadcastRoom(room); }
        // Intentionally no sender identity in the shared state or event log.
        break;
      }
      case "safety": {
        const { room, playerId } = attached;
