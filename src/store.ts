...
export const store = {
  ...
  requestPause() { if (state.connected) socket.send({ t: "pauseRequest" }); },
  ...
  saveInsight(key: keyof WorkshopInsights, value: string, previous: string) { if (state.connected) socket.send({ t: "harvest", kind: "insight", key, value, previous }); },
  saveDebriefField(index: number, value: string, previous: string) { if (state.connected) socket.send({ t: "harvest", kind: "debriefField", index, value, previous }); },
  addAction() { if (state.connected) socket.send({ t: "harvest", kind: "actionAdd" }); },
  saveActionField(id: string, key: keyof ActionItem, value: string | boolean, previous: string | boolean) { if (state.connected) socket.send({ t: "harvest", kind: "actionField", id, key, value, previous }); },
  removeAction(action: ActionItem) { if (state.connected) socket.send({ t: "harvest", kind: "actionRemove", id: action.id, previous: action }); },
