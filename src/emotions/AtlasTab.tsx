  ...
  return (
    <div className="space-y-6" key={version}>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <h2 className="font-display text-lg font-bold text-white"><T value={{ ro: "Teritoriile vizitate", en: "Territories visited" }} /></h2>
          ...
          <div className="mt-4 space-y-2">
            {families.map(({ f, count }) => (
              <div key={f.id} className="flex items-center gap-2">
                ...
                <span className="w-28 shrink-0 truncate text-xs font-semibold text-ink-200"><T value={f.name} /></span>
                ...
                <span className="w-5 text-right text-xs font-bold text-white">{count}</span>
              </div>
            ))}
          </div>
          {stats.total === 0 && <p className="mt-3 text-sm text-ink-500"><T value={{ ro: "Atlasul e gol. Numește prima emoție din Hartă sau Meteo.", en: "The atlas is empty. Name your first emotion in Map or Weather." }} /></p>}
        </Card>

        <Card delay={0.08}>
          <h2 className="font-display text-lg font-bold text-white"><T value={{ ro: "Ce pot și ce nu pot aceste numere", en: "What these numbers can and cannot do" }} /></h2>
          <ul className="mt-3 space-y-2 text-sm leading-relaxed text-ink-200">
          ...
          <p className="mt-4 rounded-xl border border-white/8 bg-white/[.03] p-3 text-xs leading-relaxed text-ink-400">
      ...
      <Card delay={0.14}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="font-display text-lg font-bold text-white"><T value={{ ro: "Datele tale", en: "Your data" }} /></h2>
            <p className="mt-1 text-xs text-ink-400">
          ...
          <div className="flex gap-2">
            ...
            <button className="btn btn-sm border border-rose-400/30 bg-rose-500/10 text-rose-200 hover:bg-rose-500/20" onClick={reset}>🗑 <T value={{ ro: "Șterge atlasul", en: "Delete atlas" }} /></button>
          </div>
        </div>
      </Card>
    </div>
...
function Stat({ icon, label, value, sub }: { icon: string; label: string; value: string; sub?: string }) {
  return (
    <div className="animate-fade-up rounded-[24px] border border-white/8 bg-ink-900/60 p-5">
