  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        <Card>
          <h2 className="font-display text-lg font-bold text-g-ink"><T value={{ ro: "O predicție nouă", en: "A new prediction" }} /></h2>
          <div className="mt-3 space-y-3">
            <div>
              <p className="mb-1 text-xs font-bold text-g-sub"><T value={{ ro: "Situația (de pe frontiera ta)", en: "The situation (on your frontier)" }} /></p>
            ...
            <div>
              <p className="mb-1 text-xs font-bold text-g-sub"><T value={{ ro: "Ce aș putea simți (opțional, 1)", en: "What might I feel (optional, 1)" }} /></p>
            ...
            <div>
              <p className="mb-1 text-xs font-bold text-g-sub"><T value={{ ro: "Ce prevăd că se va întâmpla?", en: "What do I predict will happen?" }} /></p>
        ...
        <div className="space-y-4">
          <Card delay={0.08}>
            <div className="flex items-center justify-between">
              <h2 className="font-display text-lg font-bold text-g-ink"><T value={{ ro: "Frontieră deschisă", en: "Open frontier" }} /></h2>
              <span className="text-xs text-g-sub">{open.length}</span>
            </div>
            {open.length === 0 && <p className="mt-2 text-sm text-g-faint"><T value={{ ro: "Nicio predicție așteptând verificare.", en: "No predictions waiting to be checked." }} /></p>}
            <div className="mt-2 space-y-2">
              {open.map((p) => (
                <div key={p.situation} className="rounded-2xl border border-g-line bg-g-soft p-4">
                  <p className="text-sm font-bold text-g-ink">{p.situation}</p>
                  <p className="mt-1 text-xs leading-relaxed text-g-sub">
                    {lang === "ro" ? "Prevăd:" : "I predict:"} <span className="text-g-ink">„{p.expect}"</span>
                    {p.expectEmotion && <span className="ml-1 text-g-sub">(<EmotionLabel id={p.expectEmotion} /> ~{p.expectIntensity}/10)</span>}
                  </p>
                  {resolving === p ? (
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      <button className="btn btn-sm border border-g-line bg-transparent text-g-faint hover:text-g-red" onClick={() => setList(removePrediction(p.situation))}>✕</button>
                    </div>
                  )}
                </div>
              ))}
          ...
          {resolved.length > 0 && (
            <Card delay={0.14}>
              <h2 className="font-display text-lg font-bold text-g-ink"><T value={{ ro: "Teritoriu cucerit", en: "Territory crossed" }} /></h2>
              ...
              <div className="mt-2 space-y-1.5">
                {resolved.slice(0, 8).map((p, i) => (
                  <div key={p.situation + i} className="rounded-xl bg-g-soft px-3 py-2">
                    <div className="flex items-center gap-2">
                      <span>{p.resolved === "confirmed" ? "🔁" : p.resolved === "partial" ? "◐" : "🌪️"}</span>
                      <p className="min-w-0 flex-1 truncate text-xs font-semibold text-g-ink">{p.situation}</p>
                      <span className="text-[10px] text-g-faint">{p.resolved}</span>
                    </div>
                    {p.actual && <p className="mt-1 text-[11px] italic leading-relaxed text-g-sub">„{p.actual}"</p>}
                  </div>
                ))}
              </div>
            </Card>
