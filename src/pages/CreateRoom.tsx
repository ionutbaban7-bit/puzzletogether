...
const CATEGORY_EMOJI: Record<string, string> = { words: "🔤", paintings: "🎨", landscapes: "🏔️", landmarks: "🗼", nature: "🌿", cities: "🏙️", coaching: "🧭" };
const CATEGORY_NAMES: Record<string, { ro: string; en: string }> = {
  words: { ro: "Puzzle-uri cu litere", en: "Letter-tile puzzles" }, paintings: { ro: "Picturi", en: "Paintings" },
  landscapes: { ro: "Peisaje", en: "Landscapes" }, landmarks: { ro: "Repere", en: "Landmarks" }, nature: { ro: "Natură", en: "Nature" }, cities: { ro: "Orașe", en: "Cities" }, coaching: { ro: "Team coaching", en: "Team coaching" },
};
...
export default function CreateRoom() {
  ...
  const selectedDifficulty = catalog?.difficulties.find((d) => d.id === difficulty) || null;
  ...
  async function create() {
    ...
    try {
      ...
      const response = await api.createRoom(chosen, selectedDifficulty?.id || "easy", name.trim(), {
      ...
      <div className="mx-auto max-w-5xl px-4 pb-20 sm:px-6">
        ...
        {step === 1 ? (
          <div className="space-y-7 animate-fade-up">
            <div>
              <h1 className="font-display text-3xl font-extrabold text-ink-900"><T value={{ ro: "Cu ce începe echipa?", en: "How will your team begin?" }} /></h1>
              <p className="mt-2 text-ink-500"><T value={{ ro: "Alege un joc vizual sau un exercițiu ghidat. Toate sesiunile pornesc într-un lobby controlat de facilitator.", en: "Choose a visual game or a guided exercise. Every session starts in a facilitator-controlled lobby." }} /></p>
            </div>
            <section>
              <div className="flex flex-wrap gap-2">
                {catalog?.categories.map((item) => <CategoryButton key={item.id} id={item.id} active={category === item.id} onClick={() => { setCategory(item.id); setPuzzleId(null); }} lang={lang} />)}
                {catalog?.coaching && <CategoryButton id="coaching" active={category === "coaching"} onClick={() => { setCategory("coaching"); setPuzzleId(null); }} lang={lang} coaching />}
              </div>
            </section>

            {category === "words" && <div className="rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm text-cyan-900"><b><T value={{ ro: "Mod actual: puzzle cu piese-literă.", en: "Current mode: letter-tile jigsaw." }} /></b>{" "}<T value={{ ro: "Punctele de pe litere sunt decorative; grila de formare și validarea cuvintelor sunt în dezvoltare.", en: "Letter points are decorative; free word-building and validation are still in development." }} /></div>}
            ...
            {category && !isCoaching && <section><h2 className="font-display text-lg font-bold text-ink-900"><T value={{ ro: "Dificultate", en: "Difficulty" }} /></h2><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{catalog?.difficulties.map((item) => <DifficultyCard key={item.id} difficulty={item} selected={difficulty === item.id} onSelect={() => setDifficulty(item.id)} lang={lang} />)}</div></section>}

            {error && <ErrorBox>{error}</ErrorBox>}
            <div className="sticky bottom-4 flex items-center justify-between gap-4 rounded-2xl border border-ink-200 bg-white/95 p-4 shadow-pop backdrop-blur"><div className="min-w-0 text-sm text-ink-500">{selectionReady ? <><b className="text-ink-900">{selectedActivity ? pick(selectedActivity.name, lang) : selectedPuzzle?.name}</b>{selectedActivity ? ` · ${selectedActivity.duration}` : ` · ${selectedDifficulty?.pieces} ${lang === "ro" ? "piese" : "pieces"}`}</> : <T value={{ ro: "Selectează o activitate", en: "Select an activity" }} />}</div><button className="btn-primary shrink-0" disabled={!selectionReady} onClick={() => setStep(2)}><T value={{ ro: "Continuă", en: "Continue" }} /> →</button></div>
          </div>
        ) : (
          <div className="card mx-auto max-w-xl p-6 animate-fade-up sm:p-8">
            ...
            <p className="mt-2 text-sm leading-relaxed text-ink-500"><T value={{ ro: "Invitații vor aștepta până apeși Start. Cronometrul nu pornește mai devreme.", en: "Guests will wait until you press Start. The session clock will not run early." }} /></p>
            ...
            <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-2xl border border-ink-200 bg-ink-50 p-4"><input type="checkbox" className="mt-1 h-4 w-4 accent-brand-600" checked={facilitatorOnly} onChange={(e) => setFacilitatorOnly(e.target.checked)} /><span><b className="block text-sm text-ink-900"><T value={{ ro: "Facilitez, nu joc", en: "I facilitate, I don't play" }} /></b><span className="mt-1 block text-xs leading-relaxed text-ink-500"><T value={{ ro: "Intri ca spectator: vezi activitatea și controalele, dar nu poți muta piese sau carduri.", en: "Join as a spectator: see the activity and controls, but do not move pieces or cards." }} /></span></span></label>
...
function CategoryButton({ id, active, onClick, lang, coaching = false }: { id: string; active: boolean; onClick: () => void; lang: "ro" | "en"; coaching?: boolean }) {
  return <button onClick={onClick} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${active ? coaching ? "border-emerald-600 bg-emerald-600 text-white" : "border-brand-600 bg-brand-600 text-white" : "border-ink-200 bg-white text-ink-700 hover:border-ink-300"}`}><span className="mr-1.5">{CATEGORY_EMOJI[id]}</span>{pick(CATEGORY_NAMES[id], lang)}</button>;
}
...
function DifficultyCard({ difficulty, selected, onSelect, lang }: { difficulty: Difficulty; selected: boolean; onSelect: () => void; lang: "ro" | "en" }) {
  const meta = DIFFICULTY_META[difficulty.id];
  return <button onClick={onSelect} className={`rounded-2xl border p-4 text-left transition ${selected ? "border-brand-600 bg-brand-50 ring-4 ring-brand-600/10" : "border-ink-200 bg-white hover:border-ink-300"}`}><b className={selected ? "text-brand-700" : "text-ink-900"}>{difficulty.name}</b><div className="mt-1 text-xs text-ink-500">{difficulty.pieces} {lang === "ro" ? "piese" : "pieces"}</div><div className="mt-2 text-[11px] text-ink-400">⏱ {meta?.minutes} · 👥 {meta?.people}</div></button>;
}
