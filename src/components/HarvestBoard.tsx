...
function SessionPostcard({ room, activity, players }: { room: RoomView; activity?: CoachingActivity; players: PlayerView[] }) {
  ...
  const summary = [
    `CoachingHub · PuzzleTogether — ${room.sessionName}`,
  ...
  function download() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = "coachinghub-rezumat-sesiune.txt"; link.click();
  ...
  return <section className="mt-7 rounded-xl border border-brand-200 bg-brand-50 p-5 text-g-ink sm:p-6" aria-label={ro ? "Rezumatul sesiunii" : "Session summary"}>
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="studio-kicker">CoachingHub · PuzzleTogether</p><h2 className="font-display mt-2 text-2xl">{ro ? "Ce luăm cu noi" : "What we take away"}</h2><p className="mt-1 text-sm text-g-sub">{ro ? "Concluziile salvate și acțiunile echipei, într-un singur loc." : "Saved reflections and team actions, in one place."}</p></div><div className="flex flex-wrap gap-2"><button onClick={copy} className="btn-primary btn-sm">{copied ? (ro ? "Copiat ✓" : "Copied ✓") : ro ? "Copiază rezumatul" : "Copy summary"}</button><button onClick={download} className="btn-secondary btn-sm">{ro ? "Descarcă .txt" : "Download .txt"}</button><button className="btn-secondary btn-sm" disabled={!datedActions(room).length} title={ro ? "Adaugă text și o dată acțiunilor pentru a le exporta." : "Add action text and a date to export."} onClick={() => downloadText(actionCalendar(room, players, lang), "coachinghub-actiuni.ics", "text/calendar;charset=utf-8")}>{ro ? "Acțiuni în calendar" : "Actions to calendar"}</button></div></div>
...
export function SharedText({ value, label, maxLength, disabled, save }: { value: string; label: string; maxLength: number; disabled: boolean; save: (value: string, previous: string) => void }) {
  const { lang } = useLang();
