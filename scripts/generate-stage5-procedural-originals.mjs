#!/usr/bin/env node
/**
 * Deterministically create the remaining reviewed procedural Stage 5 artwork.
 * The old isometric/abstract recipe sets were deliberately retired after a
 * contact-sheet quality review and are never generated by this script. The drawing source is this dependency-free script; it writes
...
// Do not add isometric/abstract arrays here. Their shared visual recipes were
// delisted on 2026-09-04; replacements require independent art direction.
...
// isometricArt/abstractArt remain below only to make historical provenance
// auditable; neither is an active generation target.
for (const [index,id] of blueprints.entries()) blueprintArt(id,index);
console.log(`Generated ${proceduralNature.length + cities.length + blueprints.length} reviewed procedural Stage 5 sources (retired recipe sets excluded).`);
