...
ok("catalog keeps seven categories (words replaced by canvas modes)", catalog.data.categories.length === 7 && !catalog.data.categories.some((c) => c.id === "words") && catalog.data.categories.some((c) => c.id === "letter-canvas") && catalog.data.categories.some((c) => c.id === "sentence-canvas"));
