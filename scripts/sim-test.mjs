...
ok("catalog keeps seven reviewed supported categories", catalog.data.categories.length === 7 && !catalog.data.categories.some((c) => ["words", "isometric-worlds", "abstract-geometry", "blueprint-architecture"].includes(c.id)) && ["letter-canvas", "sentence-canvas"].every((id) => catalog.data.categories.some((category) => category.id === id)));
